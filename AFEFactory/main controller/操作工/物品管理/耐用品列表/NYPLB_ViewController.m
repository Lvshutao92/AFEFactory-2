...
@interface NYPLB_ViewController ()<UITableViewDelegate,UITableViewDataSource,UIGestureRecognizerDelegate,UITextFieldDelegate>
{
    CGFloat height;
    NSInteger page;
    NSInteger totalnum;
...
@implementation NYPLB_ViewController
...
- (void)lodd{
    ...
    [session POST:KURLNSString(@"servlet/officegoods/officegoodsdurable") parameters:dic constructingBodyWithBlock:nil progress:^(NSProgress * _Nonnull uploadProgress) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([tableView isEqual:self.tableview]) {
        ...
        NYPLB_details_ViewController *vc = [[NYPLB_details_ViewController alloc] init];
        vc.idstr = model.id;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.lab1.text = [NSString stringWithFormat:@"类别编号：%@",model.code];
    
    cell.lab2.text = [NSString stringWithFormat:@"名称：%@",model.name];
    
    cell.lab3.text = [NSString stringWithFormat:@"类别：%@",model.durable_type];
    
    if ([model.need_check isEqualToString:@"1"]) {
        cell.lab4.text = [NSString stringWithFormat:@"需要盘点：%@",@"是"];
    }else if ([model.need_check isEqualToString:@"0"]) {
        cell.lab4.text = [NSString stringWithFormat:@"需要盘点：%@",@"否"];
    }
    
    
    cell.lab5.text = [NSString stringWithFormat:@"品牌：%@",model.brand];
    
    
    
    cell.lab6.text = [NSString stringWithFormat:@"型号：%@     价值(元)：%@",model.model_s,model.price];
    cell.lab7.text = [NSString stringWithFormat:@"总数：%@     闲置数：%@",model.goods_num,model.stock];
    cell.lab8.text = [NSString stringWithFormat:@"单位：%@     规格：%@",model.unit,model.standard];
...
- (void)loddeList{
    ...
    [session POST:KURLNSString(@"servlet/officegoods/officegoodsdurable/list") parameters:dic constructingBodyWithBlock:nil progress:^(NSProgress * _Nonnull uploadProgress) {
    } success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        NSDictionary *dic = [Manager returndictiondata:responseObject];
//        NSLog(@"%@",dic);
