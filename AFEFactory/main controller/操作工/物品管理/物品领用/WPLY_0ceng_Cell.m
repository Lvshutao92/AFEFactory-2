...
@end
@implementation WPLY_0ceng_Cell
...
{
        [self.imageView_cell sd_setImageWithURL:[NSURL URLWithString:NSString(model.picture)]placeholderImage:[UIImage imageNamed:@"user"]];
    ...
    self.lab5.text = [NSString stringWithFormat:@"物品类别：%@",model.durableType];
    
    
    
    if (model.goodsItemId != nil) {
        self.lab6.text = [NSString stringWithFormat:@"库存：%@",@"1"];
    }else{
        if ([model.stock integerValue] > 0){
            self.lab6.text = [NSString stringWithFormat:@"库存：%@",model.stock];
        }
    }
    
    
    self.lab7.text = [NSString stringWithFormat:@"单位：%@",model.unit];
    self.lab8.text = [NSString stringWithFormat:@"规格：%@",model.standard];
    
    self.selectBtn.selected = self.isSelected;
//    model.number = [model.model1.quantity integerValue];
...
-(void)setupMainView
{
    ...
    bgView.frame = CGRectMake(0, 0, SCREEN_WIDTH, 280);
    ...
    self.selectBtn.frame = CGRectMake(5, 252/2, 28, 28);
    self.selectBtn.selected = self.isSelected;
