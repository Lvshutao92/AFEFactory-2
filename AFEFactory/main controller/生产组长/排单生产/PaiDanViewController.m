...
#import "PaiDan_dscd_ViewController.h"


#import "PaidanjihuaSearch_ViewController.h"
#import "SYDJ_search_ViewController.h"
@interface PaiDanViewController ()<SGTopScrollMenuDelegate, UIScrollViewDelegate>
...
//    self.view.backgroundColor = [UIColor colorWithWhite:0.9 alpha:1.0];
   ...
   // self.titles = @[@"批量订单",@"排单参数", @"假日设置", @"排单计划", @"小组管理", @"所有单据"];
    ...
    NSNotification *notification =[NSNotification notificationWithName:@"appear" object:nil userInfo:dict];
    [[NSNotificationCenter defaultCenter] postNotification:notification];
    
    
     PaiDan_pdjh_ViewController *oneVC = [[PaiDan_pdjh_ViewController alloc] init];
    [self.mainScrollView addSubview:oneVC.view];
    
    [self.view insertSubview:_mainScrollView belowSubview:_topScrollMenu];
    

    if ([Manager sharedManager].searchIndex != 1 && [Manager sharedManager].searchIndex != 2) {
...
- (void)setupChildViewController {
    ...
    PaiDan_dscd_ViewController *twoVC = [[PaiDan_dscd_ViewController alloc] init];
    [self addChildViewController:twoVC];
    
    PaiDan_dwcd_ViewController *threeVC = [[PaiDan_dwcd_ViewController alloc] init];
    [self addChildViewController:threeVC];
    
    PaiDan_sydj_ViewController *sixVC = [[PaiDan_sydj_ViewController alloc] init];
