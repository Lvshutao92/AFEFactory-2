//
//  BJRKD_search_list_ViewController.m
//  Factory
//
//  Created by ilovedxracer on 2017/11/29.
...
#import "Edit_ChaCheGong_ViewController.h"
@interface BJRKD_search_list_ViewController ()<UITableViewDelegate,UITableViewDataSource>
{
    CGFloat height;
    NSInteger page;
    NSInteger totalnum;
...
@implementation BJRKD_search_list_ViewController
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (([model.stockInStatus isEqualToString:@"finished"] || [model.stockInStatus isEqualToString:@"canceled"] || [model.stockInStatus isEqualToString:@"returned"]) && ![model.stockInStatus isEqualToString:@"finished"]) {
        return 370;
    }
    return 410;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (model.receivePerson_model.personCode == nil) {
        cell.lab6.text = [NSString stringWithFormat:@"入库操作人员：%@",@"-"];
    }else{
    ...
    if (![model.stockInStatus isEqualToString:@"finished"]) {
        cell.btn.hidden = YES;
    ...
    if (cell.btn.hidden == YES && cell.btn1.hidden == YES) {
...
- (void)clickbtn:(UIButton *)sender{
    ...
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"确认取消该入库单吗？" message:@"温馨提示" preferredStyle:1];
    UIAlertAction *sure = [UIAlertAction actionWithTitle:@"确认" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        [session POST:KURLNSString(@"servlet/stock/stockinorder/cancel") parameters:dic constructingBodyWithBlock:nil progress:^(NSProgress * _Nonnull uploadProgress) {
        } success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
            NSDictionary *dic = [Manager returndictiondata:responseObject];
            //            NSLog(@"----%@",dic);
            if ([[dic objectForKey:@"code"] isEqualToString:@"success"]) {
                UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"退货成功，待重新生成采购单" message:@"温馨提示" preferredStyle:1];
...
- (void)loddeList{
    ...
    dic = @{@"businessId":[Manager redingwenjianming:@"bianhao.text"],
            ...
            @"stockInStatus":self.str4,
            @"stockInOrderNo":self.str1,
            @"orderNo":self.str2,
            @"purchaseOrderNo":self.str3,
            @"planProductionBeginTime":self.str5,
            @"planProductionEndTime":self.str6,
            @"purchaseOrder.supplierNo":self.str7,
            @"purchaseOrder.manual":self.str8,
            @"invoice":self.str9,
            };
