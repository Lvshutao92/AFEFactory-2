//
//  BJRKD_details_TableViewController.m
//  Factory
//
...
@interface BJRKD_details_TableViewController ()
{
    CGFloat height;
    CGFloat height1;
}
...
@implementation BJRKD_details_TableViewController
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0) {
        ...
        if ([self.productSku isEqualToString:@"xhrkd"]) {
            cell.lab1.text = [NSString stringWithFormat:@"商品编号：%@",model.partNo];
            cell.lab2.text = [NSString stringWithFormat:@"商品名称：%@",model.productSku_model.name];
        }else{
        ...
        if (model.location == nil) {
            cell.lab4.text = [NSString stringWithFormat:@"存放位置：%@",@"-"];
        }else{
            cell.lab4.text = [NSString stringWithFormat:@"存放位置：%@",model.location];
        }
...
- (void)lodOrderDetails{
    ...
    [session POST:KURLNSString(@"servlet/stock/stockinorderitem/item/list") parameters:dic constructingBodyWithBlock:nil progress:^(NSProgress * _Nonnull uploadProgress) {
        ...
        for (NSDictionary *dict in arr) {
            ...
            PaidanModel1 *model2 = [PaidanModel1 mj_objectWithKeyValues:model.productSku];
            model.productSku_model = model2;
