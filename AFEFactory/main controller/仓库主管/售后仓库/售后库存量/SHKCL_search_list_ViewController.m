//
//  SHKCL_search_list_ViewController.m
//  AFEFactory
//
...
#import "HCKCL_Cell.h"
@interface SHKCL_search_list_ViewController ()<UITableViewDelegate,UITableViewDataSource>
{
    CGFloat height;
    NSInteger page;
    NSInteger totalnum;
...
@implementation SHKCL_search_list_ViewController
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (model.min_storage_safe == nil) {
        cell.lab6.text = [NSString stringWithFormat:@"最低安全库存：%@",@"-"];
    }else{
        cell.lab6.text = [NSString stringWithFormat:@"最低安全库存：%@",model.min_storage_safe];
    }
    if (model.max_storage_safe == nil) {
        cell.lab7.text = [NSString stringWithFormat:@"最高安全库存：%@",@"-"];
    }else{
...
- (void)loddeList{
    ...
    [session POST:KURLNSString(@"servlet/warehouse/inventory/aftersale/list") parameters:dic constructingBodyWithBlock:nil progress:^(NSProgress * _Nonnull uploadProgress) {
