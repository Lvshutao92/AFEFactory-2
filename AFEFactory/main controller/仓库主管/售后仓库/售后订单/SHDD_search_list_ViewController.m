//
//  SHDD_search_list_ViewController.m
//  AFEFactory
//
...
@interface SHDD_search_list_ViewController ()<UITableViewDelegate,UITableViewDataSource>
{
    CGFloat height;
    NSInteger page;
    NSInteger totalnum;
...
@implementation SHDD_search_list_ViewController
...
- (void)viewDidLoad {
    ...
    [self setUpReflash];
//    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(viewappear:) name:@"SHDD_appear" object:nil];
}
...
- (void)loddeList{
    ...
    dic = @{@"businessId":[Manager redingwenjianming:@"bianhao.text"],
            ...
            @"sort":@"createTime",
            @"orderNo":self.str1,
            @"dealerName":self.str2,
            @"field7":self.str3,
            @"orderStatus":self.str4,
            };
    [session POST:KURLNSString(@"servlet/order/parts/list") parameters:dic constructingBodyWithBlock:nil progress:^(NSProgress * _Nonnull uploadProgress) {
    } success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        NSDictionary *dic = [Manager returndictiondata:responseObject];
        //      NSLog(@"%@",dic);
