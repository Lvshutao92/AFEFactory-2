//
//  XHDD_details_TableViewController.m
//  Factory
//
...
@interface XHDD_details_TableViewController ()
{
    CGFloat height;
    CGFloat height1;
}
...
@implementation XHDD_details_TableViewController
...
- (void)lodLogDetails{
    ...
    [session POST:KURLNSString(@"servlet/order/orderlog/log/list") parameters:dic constructingBodyWithBlock:nil progress:^(NSProgress * _Nonnull uploadProgress) {
    } success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        NSDictionary *dic = [Manager returndictiondata:responseObject];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    if (section == 0) {
        label.text = [NSString stringWithFormat:@"订单明细"];
        return view;
    }
