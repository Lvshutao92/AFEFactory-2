//
//  SYSB_add_edit_ViewController.m
//  AFEFactory
//
...
@interface SYSB_add_edit_ViewController ()<UITextFieldDelegate,UITableViewDelegate,UITableViewDataSource,UIGestureRecognizerDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate>
{
    UITextField *text1;
    UITextField *text2;
    UITextField *text3;
...
@implementation SYSB_add_edit_ViewController
...
- (void)clicksave{
    if (statu1.length != 0 && statu2.length != 0 && statu3.length != 0  && statu4.length != 0 &&
        text2.text.length != 0 && text3.text.length != 0 && text4.text.length != 0 &&
        text5.text.length != 0 && text7.text.length != 0 && text9.text.length != 0 &&
        ...
        dic = @{@"businessId":[Manager redingwenjianming:@"bianhao.text"],
                ...
                @"field1":text2.text,
                @"equipmentNo":text3.text,
                @"equipmentName":text4.text,
                @"equipmentColor":text5.text,
                
                @"equipmentManager":statu2,
                @"field2":text7.text,
                @"equipmentSupplierId":statu3,
                @"equipmentUseTime":text9.text,
                @"equipmentStatus":statu4,
                };
        [session POST:KURLNSString(@"servlet/equipment/equipment/add") parameters:dic constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
...
- (void)viewDidLoad {
    ...
    NSArray *arr = @[@"设备类别",@"设备规格",@"设备编号",@"设备名称",@"设备颜色",@"设备管理员",@"设备使用人",@"设备维修商",@"开始使用时间",@"设备状态"];
    ...
    UILabel *lab1 = [[UILabel alloc]initWithFrame:CGRectMake(5, 10+500, 120, 30)];
    lab1.text = @"上传设备图片";
    [self.scrollview addSubview:lab1];
    img1 = [[UIImageView alloc]initWithFrame:CGRectMake(135, 510, 100, 100)];
    img1.contentMode = UIViewContentModeScaleAspectFit;
    img1.image = [UIImage imageNamed:@"上传图片-2"];
    ...
    UILabel *lab2 = [[UILabel alloc]initWithFrame:CGRectMake(5, 630, 130, 30)];
    lab2.text = @"上传设备标识图";
    [self.scrollview addSubview:lab2];
    img2 = [[UIImageView alloc]initWithFrame:CGRectMake(135, 630, 100, 100)];
    img2.contentMode = UIViewContentModeScaleAspectFit;
    img2.image = [UIImage imageNamed:@"上传图片-2"];
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
    ...
    if ([imgstr isEqualToString:@"/pic.png"]) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([self.dataArray isEqualToArray:self.arr2]) {
        ...
        text8.text = model.supplierName;
        statu3 = model.id;
    }
    if ([self.dataArray isEqualToArray:self.arr3]) {
        PaiDanModel *model = [self.arr3 objectAtIndex:indexPath.row];
        text6.text = model.realName;
        statu2 = model.id;
    }
    if ([self.dataArray isEqualToArray:self.arr4]) {
        text10.text = [self.arr4 objectAtIndex:indexPath.row];
        if ([text10.text isEqualToString:@"正常"]) {
            statu4 = @"A";
        }else if ([text10.text isEqualToString:@"停用"]) {
            statu4 = @"B";
        }
    }
    bgwindowview.hidden = YES;
