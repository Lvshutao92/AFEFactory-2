//
//  WodeBaoyang_edit_ViewController.m
//  AFEFactory
//
...
@interface WodeBaoyang_edit_ViewController ()<UITextFieldDelegate,UIGestureRecognizerDelegate,UITextViewDelegate,UITableViewDataSource,UITableViewDelegate>
{
...
@implementation WodeBaoyang_edit_ViewController
...
- (void)clicksave{
    ...
    //content:维修申请
    if (text3.text.length != 0 && textview.text.length != 0 &&
        idstring1.length != 0  && idstring3.length != 0) {
        AFHTTPSessionManager *session = [Manager returnsession];
        __weak typeof(self) weakSelf = self;
        NSDictionary *dic = [[NSDictionary alloc]init];
        dic = @{@"businessId":[Manager redingwenjianming:@"bianhao.text"],
        ...
        NSString *str;
        str = @"servlet/equipment/equipmentmaintain/update";
        [session POST:KURLNSString(str) parameters:dic constructingBodyWithBlock:nil progress:^(NSProgress * _Nonnull uploadProgress) {
            ...
            if ([[dic objectForKey:@"code"] isEqualToString:@"success"]) {
                ...
                UIAlertAction *cancel = [UIAlertAction actionWithTitle:@"OK" style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
                    ...
                    NSNotification *notification =[NSNotification notificationWithName:@"save-baoyang-edit" object:nil userInfo:dict];
