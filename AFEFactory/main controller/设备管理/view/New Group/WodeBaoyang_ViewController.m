...
#import "WodeBaoyang_edit_ViewController.h"

#import "BaoYangMingXi_ViewController.h"
@interface WodeBaoyang_ViewController ()<UITextFieldDelegate,UITableViewDelegate,UITableViewDataSource,UIGestureRecognizerDelegate>
{
    CGFloat height;
    NSInteger page;
    NSInteger totalnum;
...
@implementation WodeBaoyang_ViewController
...
- (void)viewDidLoad {
    ...
    [self setupview_tableview];
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(save:) name:@"save-baoyang-edit" object:nil];
}
...
- (void)clickbtn1:(UIButton *)sender{
    ...
    PaiDanModel *model = [self.dataArray objectAtIndex:indexpath.row];
        ...
        [session POST:KURLNSString(@"servlet/equipment/equipmentmaintain/delete") parameters:dic constructingBodyWithBlock:nil progress:^(NSProgress * _Nonnull uploadProgress) {
        } success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
            NSDictionary *dic = [Manager returndictiondata:responseObject];
            //NSLog(@"---  ---%@",dic);
...
- (void)clickbtn2:(UIButton *)sender{
    ...
    WodeBaoyang_edit_ViewController *edit = [[WodeBaoyang_edit_ViewController alloc]init];
