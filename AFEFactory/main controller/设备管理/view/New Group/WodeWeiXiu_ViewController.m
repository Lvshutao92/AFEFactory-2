//
//  WodeWeiXiu_ViewController.m
//  AFEFactory
//
//  Created by ilovedxracer on 2018/1/17.
...
@interface WodeWeiXiu_ViewController ()<UITextFieldDelegate,UITableViewDelegate,UITableViewDataSource,UIGestureRecognizerDelegate>
{
    CGFloat height;
    NSInteger page;
    NSInteger totalnum;
...
@implementation WodeWeiXiu_ViewController
...
- (void)viewDidLoad {
    ...
    idstring2 = @"";
    idstring3 = @"";
    self.arr3 = [@[@"新建",@"已审核",@"未通过",@"维修成功",@"维修失败",@"已生成付款单"]mutableCopy];
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(save:) name:@"saveedit" object:nil];
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if ([tableView isEqual:self.tableview1]) {
        ...
        if ([self.dataArray1 isEqualToArray:self.arr3]) {
            ...
            if ([text6.text isEqualToString:@"新建"]) {
                ...
                idstring3 = @"C";
            }else if ([text6.text isEqualToString:@"维修成功"]) {
                idstring3 = @"D";
            }else if ([text6.text isEqualToString:@"维修失败"]) {
                idstring3 = @"E";
    ...
    if ([tableView isEqual:self.tableview]) {
        ...
        WeiXiuMingXi_ViewController *mingxi = [[WeiXiuMingXi_ViewController alloc]init];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (model.field1 == nil) {
        ...
        cell.lab11.text = model.field1;
    }
    
    if (model.field3 == nil) {
        cell.lab12.text = @"-";
    }else{
        cell.lab12.text = model.field3;
    }
    
    if (model.field5 == nil) {
        cell.lab13.text = @"-";
    }else{
        cell.lab13.text = model.field5;
    }
    
    if (model.field4 == nil) {
        cell.lab14.text = @"-";
    }else{
        cell.lab14.text = model.field4;
    }
...
- (void)clickbtn1:(UIButton *)sender{
    ...
    [session POST:KURLNSString(@"servlet/equipment/equipmentmaintenance/delete") parameters:dic constructingBodyWithBlock:nil progress:^(NSProgress * _Nonnull uploadProgress) {
    } success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        NSDictionary *dic = [Manager returndictiondata:responseObject];
        //NSLog(@"---  ---%@",dic);
...
- (void)clickbtn2:(UIButton *)sender{
    ...
    WodeWeixiu_edit_ViewController *edit = [[WodeWeixiu_edit_ViewController alloc]init];
    edit.navigationItem.title = @"修改";
    edit.idstr = model.id;
    
    edit.str1 = model.field2;
...
- (void)loddeList{
    ...
    [session POST:KURLNSString(@"servlet/equipment/equipmentmaintenance/mylist") parameters:dic constructingBodyWithBlock:nil progress:^(NSProgress * _Nonnull uploadProgress) {
    } success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        NSDictionary *dic = [Manager returndictiondata:responseObject];
        //                NSLog(@"guowai----%@",dic);
